...
Zive.prototype.foundationHandler = async function (msg) {
    ...
    switch (cmdName) {
        ...
        case 'write':
            ...
            break;
        case 'writeNoRsp':
            await this._foundApis.write(clusters, cId, data.payload, msg);
            break;
        case 'readStruct':
        case 'writeStrcut':
            // not support now
    ...
    if (cmdRsp){
        const result = await this._foundApis[cmdName](clusters, cId, data.payload, msg)
        await this.foundation(null, msg.srcaddr, msg.srcendpoint, cId, cmdRsp, result, cfg)
    }
};
...
Zive.prototype._report = async function (cId, attrId, data, afMsg) {
    var cfg = {
    ...
    await this.foundation(null, afMsg.srcaddr, afMsg.srcendpoint, cId, 'report', attrReport, cfg);
};
