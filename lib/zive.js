...
Zive.prototype.functionalHandler = async function (msg, remoteEp) {
    ...
    try {
        ...
        await this.foundation(msg.srcaddr, msg.srcendpoint, cId, 'defaultRsp', payload, defaultCfg)
        throw err
    }
    
    console.log({cmdRspName, defaultRsp})
    if (cmdRspName) {
        if (!rspData) {
            if (defaultRsp === 0) {
            ...
            payload = rspData;
            await this.functional(msg.srcaddr, msg.srcendpoint, cId, cmdRspName, payload, cfg);
